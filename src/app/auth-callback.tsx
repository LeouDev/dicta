...
/** Target of the email-confirmation link (dicta://auth-callback?code=…). */
export default function AuthCallbackScreen() {
  ...
  useEffect(() => {
    ...
    exchangeAuthCode(code).then(
      () => router.replace('/'),
      (e) => setError(friendlyError(e, EXPIRED)),
    );
  }, [code]);
  ...
  return (
    <Screen contentStyle={styles.center}>
      {error ? (
        <EmptyState title="Link expired" message={error} actionLabel="Continue" onAction={() => router.replace('/')} />
      ) : (
